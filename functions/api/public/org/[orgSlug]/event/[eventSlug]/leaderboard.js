...
export async function onRequestGet(context) {
  ...
  const parMap = {};
  let totalPar = 0;
  eventHoles.forEach(h => {
    parMap[h.hole_number] = h.par;
    totalPar += h.par;
  });

  // If leaderboard hidden, return event info but no teams
  ...
  const { results: teams } = await db.prepare(
    'SELECT id, team_name, created_at FROM teams WHERE event_id = ? ORDER BY created_at'
  ).bind(event.id).all();
  ...
  // Sort: to_par ASC, holes_completed DESC, last_updated ASC
